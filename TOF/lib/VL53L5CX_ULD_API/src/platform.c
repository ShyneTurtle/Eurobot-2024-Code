		...
		uint8_t *p_value)
{
	...
	//Lecture
	i2c_read_blocking(p_platform->i2c, p_platform->address, regAddress, 2, true);
	status = i2c_write_blocking(p_platform->i2c, p_platform->address, p_value, 1, false);

	if (status != PICO_ERROR_GENERIC)		status = 0;
		...
		uint8_t value)
{
	...
	status = i2c_write_blocking(p_platform->i2c, p_platform->address, data, 3, false);
		...
		uint32_t size)
{
	...
	data[1] = (uint8_t)(registerAddress & 0x00ff);
 	for (int i = 2; i < size + 2; i++)		data[i] = p_values[i];

	//Tente d'écrire plusieurs octets
	status = i2c_write_blocking(p_platform->i2c, p_platform->address, data, size + 2, false);
		...
		uint32_t size)
{
	...
	uint8_t regAddress[2] = { (uint8_t)(registerAddress >> 8), (uint8_t)(registerAddress & 0x00ff) };
	p_values = (uint8_t*)malloc(size * sizeof(uint8_t));
